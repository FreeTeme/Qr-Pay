...
from models import Session, Business, User, UserBusiness
...
# Клавиатура для уведомления бизнесу
def get_payment_keyboard(user_id: int, business_id: int):
    builder = InlineKeyboardBuilder()
    builder.button(
        text="💳 Оформить покупку",
        callback_data=f"process_payment:{user_id}:{business_id}"  # Используем ":"
    )
...
async def cmd_start(message: types.Message, command: CommandObject):
    ...
    try:
        if args and args.startswith("business_"):
            ...
            # Получаем бизнес (современный метод)
